...
public class addFirstPlayerToMyTeam  extends MyInternalFrame {
	...
	public addFirstPlayerToMyTeam(User u){
		super("Coach adds a FirstPlayer");
		this.user = u;
	...
	public void actionPerformed(ActionEvent arg0) {
		try {
			...
			int playerID = PlayersComboBox.getItemAt(PlayersComboBox.getSelectedIndex());
				if(sysData.addPlayerToTeamFirstPlayers(playerID, teamID)) {
					messageBox.showInternalInformationMessage("Player " + playerID +
							"\nhas successfully been added from First Team " + teamID);
				}
				else {
					messageBox.showInternalErrorMessage("Failed to add Player " + playerID +
							"\nto First Team " + teamID);
				}
		}
		}
		catch(Exception e) {
