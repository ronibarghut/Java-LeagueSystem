...
public class removePlayerFromTeam extends MyInternalFrame {
	...
	public removePlayerFromTeam(User u){
		super("Coach Removes a Player");
		this.user = u;
	...
	public void actionPerformed(ActionEvent arg0) {
		try {
			...
			int playerID = PlayersComboBox.getItemAt(PlayersComboBox.getSelectedIndex());
				if(sysData.RemovePlayerFomTeam(playerID, teamID)) {
					messageBox.showInternalInformationMessage("Player " + playerID +
							"\nhas successfully been removed from Team " + teamID);
				}
				else {
