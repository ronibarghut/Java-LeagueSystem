...
public class removePlayerFromFirstTeam extends MyInternalFrame {
	...
	public removePlayerFromFirstTeam(User u){
		super("Coach Removes a FirstPlayer");
		this.user = u;
	...
	public void actionPerformed(ActionEvent arg0) {
		try {
			...
			int playerID = PlayersComboBox.getItemAt(PlayersComboBox.getSelectedIndex());
				if(sysData.removePlayerfromTeamFirstPlayers(playerID, teamID)) {
					messageBox.showInternalInformationMessage("Player " + playerID +
							"\nhas successfully been removed from FirstPayerTeam " + teamID);
				}
				else {
					messageBox.showInternalErrorMessage("Failed to remove Player " + playerID +
							"\nfrom FirstPayerTeam " + teamID);
				}
		}
		}
		catch(Exception e) {
