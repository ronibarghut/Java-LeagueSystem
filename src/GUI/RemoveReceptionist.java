...
public class RemoveReceptionist  extends MyInternalFrame {
	
	private static final long serialVersionUID = 1L;
	
	private javax.swing.JLabel jLabel1 = new javax.swing.JLabel();
	private JComboBox<Integer> RecepComboBox = new JComboBox<Integer>(sysData.getRecepIDS());

	
	
	
	public RemoveReceptionist() {
		super("Remove Receptionist");
	    ...
	    final JLabel StadLabel = new JLabel("Receptionist ID:");
		...
		RecepComboBox.setFont(new Font("Dialog", Font.BOLD, 12));
		RecepComboBox.setBounds(265, 71, 241, 20);
		jLabel1.add(RecepComboBox);
	    
		
		
		JButton RemoveButton = new JButton("Remove Receptionist");
	...
	public void actionPerformed(ActionEvent arg0) {
		try {
			...
			else if(arg0.getActionCommand().equals("Remove Receptionist")){
			if(RecepComboBox.getSelectedIndex() == -1) {
				throw new Exception("No Receptionist selected!");
			}
			int RecepID = RecepComboBox.getItemAt(RecepComboBox.getSelectedIndex());
				if(sysData.removeReceptionist(RecepID)) {
					messageBox.showInternalInformationMessage("Receptionist " + RecepID +
							"\nhas successfully been removed " );
				}
				else {
					messageBox.showInternalErrorMessage("Failed to Remove Receptionist " + RecepID  );
				}
