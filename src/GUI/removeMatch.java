...
public class removeMatch  extends MyInternalFrame {
	...
	public removeMatch() {
	    ...
	    final JLabel StadLabel = new JLabel("Match Number:");
		...
		JButton RemoveButton = new JButton("Remove Match");
	...
	public void actionPerformed(ActionEvent arg0) {
		try {
			...
			else if(arg0.getActionCommand().equals("Remove Match")){
			if(MatchComboBox.getSelectedIndex() == -1) {
				throw new Exception("No Match selected!");
			}
			int MatchNum = MatchComboBox.getItemAt(MatchComboBox.getSelectedIndex());
				if(sysData.removeMatch(MatchNum)) {
					messageBox.showInternalInformationMessage("Match " + MatchNum +
							"\nhas successfully been removed " );
				}
				else {
					messageBox.showInternalErrorMessage("Failed to Remove Match " + MatchNum  );
				}
